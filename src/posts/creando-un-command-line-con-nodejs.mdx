...
Eso fue lo que hice ya hace creo +1 de año, si te interesa de frente el codigo, ahi ta: https://github.com/ouracademy/website/tree/master/template
...
const title = "un titulo muy largoooooooooooooooooooooooooooooooooooooooooooooooooooooooooooooooooooooooooooooooooooooooooooooooooooo"
...
const {error} = Joi.string().max(60).messages({"string.max": sugerenciaSEO }).validate(title)
console.log(error.message) 
  ...
  .prompt([
    {
      name: "title",
      message: "Por favor, dame el titulo de tu post:",
      validate: title => { // aca la integracion con joi :)
...
Eso es todo por este post, espero que te haya gustado, el codigo completo (el que uso en mi blog), esta en https://github.com/ouracademy/website/tree/master/template
